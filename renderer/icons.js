// Inlined Lucide icon paths (MIT, lucide.dev) + Cue logo glyph.
(function () {
  const P = new Map([
  ...
  function icon(name, opts) {
    ...
    if (FILLED.has(name)) {
      return '<svg viewBox="0 0 24 24" width="' + size + '" height="' + size + '" fill="currentColor" stroke="none" xmlns="http://www.w3.org/2000/svg">' + FILLED.get(name) + '</svg>';
    }
    const d = P.has(name) ? P.get(name) : '';
